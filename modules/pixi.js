...
// const effectIconsPerRowSetting = module.settings.register("effectIconsPerRow", Number, 6, {
//   hasHint: true,
//   onChange: refresh,
...
const updateIconPosition = (effectIcon, i, effectIcons, token) => {
  ...
  // effectIcon.position.x = horizontal ? x + x : y + y;
  // effectIcon.position.y = horizontal ? y + y : x + x;
  const ratio = i / effectIcons.length;
  const { x, y } = polar_to_cartesian(100, ratio * 2 * Math.PI);
  // console.log({ x, y });
...
const updateEffectScales = (token) => {
  ...
  if (numEffects > 0 && token.effects.children.length > 0) {
    ...
    effectIcons.forEach((effectIcon, i, effectIcons) => {
      ...
      // mask.drawCircle(effectIcon.position.x, effectIcon.position.y, effectIcon.width / 2);
      // mask.endFill();

      // effectIcon.mask = mask;
      // let redMask = new PIXI.Sprite(0xff0000);
      // redMask.height = 10000;
      // redMask.width = 10000;
      // effectIcon.mask = redMask;
      updateIconSize(effectIcon, 36);
